...
#creating table
creating_table='''CREATE TABLE IF NOT EXISTS userscore(
...
insert_values = '''
INSERT INTO userscore(name, level, score)
VALUES (%s, %s, %s)
ON CONFLICT (name) DO UPDATE SET level = EXCLUDED.level, score = EXCLUDED.score
'''
cur.execute('SELECT score, level FROM userscore WHERE name = %s', (n,))
a = cur.fetchone()

if a:
    old_indicator_score = a[0]
    old_level = a[1]
    print(f"Welcome back, {n}. Score: {old_indicator_score}, Level: {old_level}")
else:
    insert_values = '''
    INSERT INTO userscore(name, level, score) VALUES (%s, %s, %s)
    '''
    cur.execute(insert_values, (n, 0, 0))
    conn.commit()
    old_indicator_score = 0
    old_level = 0
    print(f"New user '{n}' created.")
...
#snake 
head =pygame.image.load('negr.png')
...
while run:
    ...
    if over_indicator:
        ...
        if keys[pygame.K_DOWN] and dy!=-1:
            ...
            dy=1
        if keys[pygame.K_s]:
            cur.execute(f'UPDATE userscore SET level={level} WHERE name=\'{n}\'')
            cur.execute(f'UPDATE userscore SET score={indicator_score} WHERE name=\'{n}\'')
            conn.commit()
        if keys[pygame.K_p]:
            record=False
    ...
    if over_indicator:
        ...
        #screen.blit(head,(body[0][0],body[0][1]))
    ...
    clock.tick(5+t)
    pygame.display.flip()
print(old_indicator_score,old_level)
if record:
    cur.execute(f'UPDATE userscore SET level={level} WHERE name=\'{n}\'')
    cur.execute(f'UPDATE userscore SET score={indicator_score} WHERE name=\'{n}\'')
